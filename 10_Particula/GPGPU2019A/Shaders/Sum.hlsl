...
// este bffer es mas basico, no puede ser una struct 
Buffer<float4> Input;
RWBuffer<float4> Output;

// Memoria compartida entre hilos, pero cada grupo cada tiene un buffer
...
void Sum(uint3 lid:SV_GroupThreadID, uint3 gid : SV_DispatchThreadID, uint3 group:SV_GroupID)
{
	...
	//No continua hasta que todos los hilos hayan leido su impout y llegaron aqui
	GroupMemoryBarrierWithGroupSync();
	if (!(lid.x & 0x01))//los pares
	{
		SharedBuffer[lid.x]+= SharedBuffer[lid.x + 1];
	}
	GroupMemoryBarrierWithGroupSync();
	if (!(lid.x & 0x03))// de a cuatro
	{
