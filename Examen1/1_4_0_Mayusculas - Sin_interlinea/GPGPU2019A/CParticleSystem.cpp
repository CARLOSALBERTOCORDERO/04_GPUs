...
void CParticleSystem::OnEndTxt(const streampos sizeOut)
{
	...
	dbd.Usage = D3D11_USAGE_STAGING;
	dbd.MiscFlags = D3D11_RESOURCE_MISC_BUFFER_STRUCTURED;
	std::vector<PARTICLE> textOut;
	textOut.resize(sizeOut);
	dbd.ByteWidth = textOut.size() * sizeof(PARTICLE);
	ID3D11Buffer* pOB = nullptr;
	m_pOwner->GetDevice()->CreateBuffer(&dbd, 0, &pOB);
	m_pOwner->GetContext()->CopyResource(pOB, m_vConstantBuffers[1]);
	D3D11_MAPPED_SUBRESOURCE dms;
	m_pOwner->GetContext()->Map(pOB, 0, D3D11_MAP_READ, 0, &dms);
	memcpy(&textOut[0], dms.pData, sizeof(PARTICLE)*textOut.size());
	m_pOwner->GetContext()->Unmap(pOB, 0);
	pOB->Release();

	ofstream file;
	file.open("..\\mayusculas.txt");
	for (int i = 0; i < textOut.size(); i++)
	{
		file << (char)textOut[i].letter;
	}
	file.close();
